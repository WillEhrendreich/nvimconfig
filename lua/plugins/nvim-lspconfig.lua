...
local mason_extra = {
  ...
  opts = {
    ensure_installed = { "roslyn", "fsautocomplete" },
    registries = {
...
local function setDevIfIonideIsLocal(spec)
  if require("config/util").hasRepoWithName("Ionide-nvim") then
    spec["dev"] = true
    spec["dir"] = require("config/util").getRepoWithNameOrDefault("Ionide-nvim", "")
    return spec
  else
    return spec
  end
      ...
      local ret = {
        ...
        setup = {
          ...
          ionide = function(server, opts)
            local ionide_config = require("ionide").setup(opts)
            local on_attach = ionide_config.on_attach
            ionide_config.on_attach = function(client, bufnr)
              on_attach(client, bufnr)
              vim.lsp.semantic_tokens.enable(false, { bufnr = bufnr, client_id = client.id })
            end
            vim.lsp.config("ionide", ionide_config)
