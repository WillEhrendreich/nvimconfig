...
function v.dotnet.GetProjectPath()
    ...
    if not v.dotnet.LastProjectFileName or v.dotnet.LastProjectFileName == "" then
        v.dotnet["LastProjectFileName"] =
            projectName
    end
    if not v.dotnet.LastProjectFileExtension or v.dotnet.LastProjectFileExtension == "" then
        v.dotnet[
...
function v.dotnet.BuildDebug(p)
    local logfile = "c:/temp/dap-debug-nvim-dotnet.log"
...
v.lsp.on_attach = function(client, bufnr)
    ...
    if not tContains(ignored, client.name) then
        ...
        lsp_mappings.n["<leader>lI"] = {
            function() vim.api.nvim_cmd({ cmd = "LspRestart" }, {}) end,
...
v.lsp.configs = {
    ...
    ionide = {
        ...
        settings = v.lsp.lspconfig.options.fsautocomplete.settings,
    },
    yamlls = {
        settings = {
            yaml = {
    ...
    -- pin_plugins
        {
                            ...
                            commands = {
                                ...
                                delete = function(state)
                                    ...
                                    local command =
                                        ...
                                        vim.fn.fnameescape(path) .. "\",'OnlyErrorDialogs','SendToRecycleBin')"
                        ...
                        highlight = {
                            ...
                            disable = function(lang, buf)
                                ...
                                if
                                    ...
                                    or
                                    ok and stats and stats.size > max_filesize then
                                    return true
                                end
                            end,
            ...
            ["hrsh7th/nvim-cmp"] = {
                ...
                config = function()
                    ...
                    setup {
                        ...
                        formatting = {
                            ...
                            function(entry, vim_item)
                                ...
                                if not prsnt then
                                    ...
                                    return (lspkind.cmp_format(v.lspkind))(entry, vim_item) or nil
                                end
                                -- Source
                                -- vim_item.menu = ({
                                --         buffer = "[Buf]",
                        ...
                        cmp.setup.filetype("fsharp_project", {
                            sources = cmp.config.sources {
                                { name = "codeium" },
                                { name = "luasnip",  priority = 1000 },
                                { name = "nvim_lsp", priority = 900 },
            ...
            ["neoconf-neovim-options"] = {
                lazy = false,
                url = "https://git.jacky.wtf/neovim/neoconf-neovim-configuration.git",
                config =
                function()
                    require("neoconf-neovim-options").setup()
                end,
                dependencies = {
                    ...
                    ["neovim/nvim-lspconfig"] = {
                        ...
                        config = function()
                            ...
                            if v.isAvalable "mason-lspconfig.nvim" then
                                vim.api.nvim_create_autocmd("User",
                                    { pattern = "VimSharpLspSetup", once = true, callback = setup_servers })
                            else
                                setup_servers()
                            end
                        end,
            ...
            ["djoshea/vim-autoread"] = {
                init = function() table.insert(v.filePlugins, "vim-autoread") end,
            },
            -- -- ["dhruvasagar/vim-table-mode"] = re "vim-table-mode",
            ["echasnovski/mini.nvim"] = {
                init = function() table.insert(v.filePlugins, "mini.nvim") end,
                config = function()
...
local mappings =
{
    n = {
        ...
        ["<leader>bb"]  = { function()
            v.status.heirline.buffer_picker(function(bufnr)
        ...
        ["<leader>o"]   = { function()
            if v.isAvalable "neo-tree.nvim" then
                ...
                if vim.bo.filetype == "neo-tree" or vim.bo.filetype == "neo-tree-popup" then
                    -- vim.notify("Filetype is neo-tree or neo-tree-popup, calling Neotree close")
                    c.execute({ action = "close" })
                else
                    -- vim.notify("Filetype is neither neo-tree nor neo-tree-popup, calling Neotree show and focus")
                    c.execute({ action = "focus" })
                end
            else
                vim.cmd("e<cr>")
            end
        ...
        -- ["<leader>uc"]  = { function() if v.isAvalable "nvim-cmp" then v.ui.toggle_cmp() end end, desc = "Toggle autocompletion" },

        -- ["<leader>uC"]  = { function() if v.isAvalable "nvim-colorizer.lua" then vim.cmd.ColorizerToggle() end end, desc = "Toggle color highlight" },
